...
function App(){
 ...
 <div className={`product-top ${product.id}`}><div className="product-copy"><p className="eyebrow">{t(product.tag)}</p>{product.id==='rallo'?<img className="rallo-mark" src={brand('rallo-white.svg')} alt="RALLO"/>:product.id==='pulse'?<div className="pulse-mark"><img src={brand('pulse-logo-horizontal.png')} alt="PULSE — Make Knowledge Count."/></div>:product.id==='phenolab'?<div className="phenolab-identity"><div className="phenolab-mark"><img src={brand('pheno-logo.png')} alt="Pheno"/></div><p className="product-name">Phenolab</p></div>:<p className="product-name">{product.name} <span>{product.cn}</span></p>}<h3>{t(product.title)}</h3><p>{t(product.intro)}</p><p className="product-for">{t(product.takeaway)}</p><a className="button white" href={product.id==='phenolab'?'https://lab.szkl.com':mailLink(`${product.name} pilot enquiry`)} target={product.id==='phenolab'?'_blank':undefined} rel={product.id==='phenolab'?'noopener noreferrer':undefined}>{t(product.link)}<ArrowUpRight size={18}/></a></div><figure className={`product-image ${product.id}`}><button className="image-button" onClick={()=>open(productImage,product.imageAlt,product.caption)} aria-label={`${t(copy.expand)}: ${product.name}`}><img src={media(productImage)} alt={t(product.imageAlt)} loading="lazy"/><span className="expand-icon"><Plus size={20}/></span></button><figcaption>{t(product.caption)}</figcaption></figure></div>
