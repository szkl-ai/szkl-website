...
import {ArrowUpRight,ArrowRight,ArrowDown,Camera,Mic,ScanLine,Workflow,Layers,Check,Link as LinkIcon,Download,Printer,X,Plus,Mail,FlaskConical,CornerDownRight} from 'lucide-react';
...
function App(){
 ...
 const [selected,setSelected]=useState(products.some(p=>p.id===params.get('application'))?params.get('application')!:'nian');
 ...
 <header className="site-header"><div className="wrap header-inner">
  ...
  <nav aria-label={t(tx('Main navigation','主导航'))}>{profile?<a href={url('/')}><ArrowRight className="back-arrow" size={15}/>{t(copy.back)}</a>:<><a href="#applications">{t(copy.applications)}</a><a href="#approach">{t(copy.approach)}</a><a href="#people">{t(copy.people)}</a></>}</nav>
 ...
 <section className="hero wrap"><div className="hero-copy"><p className="eyebrow"><span className="blue-rule"/>{t(copy.eyebrow)}</p><h1>{t(copy.heroTitle)}</h1><p className="lead">{t(copy.heroLead)}</p><p className="hero-sub">{t(copy.heroSub)}</p><div className="actions"><a className="button primary" href="#applications">{t(copy.explore)}<ArrowDown size={18}/></a><a className="text-link" href="#contact">{t(copy.contact)}<ArrowUpRight size={18}/></a></div></div>
 <div className="hero-visual"><div className="visual-label"><span>SZKL / {t(tx('Field of work','应用图谱'))}</span><span>01—04</span></div><a href="#applications" onClick={()=>select('labpilot')} className="hero-main-image"><img src={media('lab-concept.png')} alt={t(products[2].imageAlt)}/><span className="image-stamp">{t(tx('Laboratory capture · concept','实验采集 · 概念'))}</span></a><div className="hero-pair"><a href="#applications" onClick={()=>select('nian')}><img src={media('nian-product.png')} alt={t(products[0].imageAlt)}/><span>Nian / {t(tx('Design rendering','设计渲染'))}</span></a><a href="#applications" onClick={()=>select('rallo')}><img src={media('sports-concept.png')} alt={t(products[1].imageAlt)}/><span>RALLO / {t(tx('Installation concept','安装概念'))}</span></a></div></div></section>
 <div className="field-strip wrap">{[tx('Voice & memory','语音与记忆'),tx('Sports & movement','运动与动作'),tx('Science & experiments','科研与实验'),tx('Enterprise & operations','企业与运营')].map((x,i)=><a key={i} href="#applications" onClick={()=>select(products[i].id)}><span>0{i+1}</span>{t(x)}<ArrowUpRight size={16}/></a>)}</div>
 <section className="loop-section section wrap" id="approach"><div className="section-heading"><p className="eyebrow">{t(copy.loopLabel)}</p><h2>{t(copy.loopTitle)}</h2><p>{t(copy.loopDesc)}</p></div><div className="loop-grid">{copy.stages.map((s,i)=>{const Icon=[Camera,ScanLine,Workflow][i];return <article key={i}><div className="stage-number"><span>0{i+1}</span><Icon size={23}/>{i<2&&<ArrowRight className="stage-arrow" size={23}/>}</div><h3>{t(s.title)}<span>{t(s.sub)}</span></h3><p>{t(s.body)}</p><div className="stage-items">{t(s.items)}</div></article>})}</div><div className="feedback"><CornerDownRight size={18}/>{t(copy.feedback)}</div></section>
 <section className="applications section" id="applications"><div className="wrap"><div className="section-heading light"><p className="eyebrow">{t(copy.appsLabel)}</p><h2>{t(copy.appsTitle)}</h2><p>{t(copy.appsLead)}</p></div><div className="product-tabs" role="tablist" aria-label={t(copy.chosen)}>{products.map((p,i)=><button key={p.id} id={`tab-${p.id}`} role="tab" aria-selected={p.id===selected} aria-controls="application-view" tabIndex={p.id===selected?0:-1} onClick={()=>select(p.id)} onKeyDown={e=>{if(['ArrowRight','ArrowLeft','Home','End'].includes(e.key)){e.preventDefault();const n=e.key==='Home'?0:e.key==='End'?3:(i+(e.key==='ArrowRight'?1:3))%4;select(products[n].id);document.getElementById(`tab-${products[n].id}`)?.focus()}}}><span className="tab-index">0{i+1}</span><span className="tab-name">{p.name}<small>{t(p.category)}</small></span><ArrowUpRight size={20}/></button>)}</div>
 <div className="application-view" id="application-view" role="tabpanel" aria-labelledby={`tab-${selected}`}>
 <div className="product-top"><div className="product-copy"><p className="eyebrow">{t(product.tag)}</p>{product.id==='rallo'?<img className="rallo-mark" src={brand('rallo-white.svg')} alt="RALLO"/>:product.id==='pulse'?<div className="pulse-mark"><img src={brand('pulse-logo-horizontal.png')} alt="PULSE — Make Knowledge Count."/></div>:<p className="product-name">{product.name} <span>{product.cn}</span></p>}<h3>{t(product.title)}</h3><p>{t(product.intro)}</p><p className="product-for">{t(product.takeaway)}</p><a className="button white" href={mailLink(`${product.name} pilot enquiry`)}>{t(product.link)}<ArrowUpRight size={18}/></a></div><figure className={`product-image ${product.id}`}><button className="image-button" onClick={()=>open(product.image,product.imageAlt,product.caption)} aria-label={`${t(copy.expand)}: ${product.name}`}><img src={media(product.image)} alt={t(product.imageAlt)} loading="lazy"/><span className="expand-icon"><Plus size={20}/></span></button><figcaption>{t(product.caption)}</figcaption></figure></div>
 ...
 <div className="product-gallery"><figure><button className="image-button secondary-image" onClick={()=>open(product.secondary,product.secondaryAlt,product.secondaryCaption)} aria-label={`${t(copy.expand)}: ${t(copy.gallery)}`}><img src={media(product.secondary)} alt={t(product.secondaryAlt)} loading="lazy"/><span className="expand-icon"><Plus size={20}/></span></button><figcaption>{t(product.secondaryCaption)}</figcaption></figure><OutputDemo kind={product.demo} lang={lang}/></div>
 <div className="product-status"><span>{t(copy.status)}</span><p>{t(product.stage)}</p></div>
 </div></div></section>
 <section className="edge section wrap"><div className="edge-heading"><p className="eyebrow">{t(tx('From capability to competitive advantage','从技术能力，到竞争优势'))}</p><h2>{t(copy.edgeTitle)}</h2><p className="lead">{t(copy.edgeLead)}</p><p>{t(copy.edgeBody)}</p></div><div className="advantage-list">{copy.advantages.map((x,i)=><div key={i}><span>0{i+1}</span><div><h3>{t(x.title)}</h3><p>{t(x.body)}</p></div></div>)}</div></section>
...
function OutputDemo({kind,lang}:{kind:string;lang:Lang}){
 ...
 label:tx('From observation to next experiment','从观察到下一轮实验'),icon:FlaskConical,title:tx('Keep the scientific trail intact.','保留完整的科研证据链。'),
 rows:[{label:tx('Record','记录'),text:tx('Link the observation to the sample and procedure step.','将观察关联到样品和具体操作步骤。')},{label:tx('Compare','比较'),text:tx('Review the conditions and outcomes across runs.','跨批次比较实验条件与结果。')},{label:tx('Propose','提出建议'),text:tx('Prepare a follow-up experiment for scientist approval.','准备后续实验建议，交由研究人员确认。')}],foot:tx('Illustrative workflow · recommendation capability in development.','工作流程示例 · 实验建议能力仍在开发中。')
 }:{
