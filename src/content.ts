...
export const products=[
 ...
 link:tx('Discuss a sports pilot','洽谈运动场景试点'),demo:'sport'
 },
 {id:'labpilot',name:'LabPilot',cn:'',category:tx('Laboratories & experiments','实验室与实验'),tag:tx('Software prototype · capture development','软件原型 · 采集方案开发'),
 title:tx('Every experiment should teach the next one.','让每一次实验，为下一次积累依据。'),
 intro:tx('Connect the observations at the bench with the sample, the procedure and the result—so experimental work becomes usable evidence.','将实验台上的观察与样品、操作步骤和结果关联起来，让实验过程形成可复用的证据。'),
 input:tx('Experimental records, instrument files and bench observations. Glovebox cameras add a proposed visual capture channel.','实验记录、仪器文件和实验台观察；手套箱相机是拟开发的视觉采集方式。'),
 model:tx('Link sample lineage, process conditions, observations and outcomes using structured data and domain models.','通过结构化数据与领域模型，关联样品谱系、工艺条件、观察和结果。'),
 action:tx('Compare runs and identify gaps. Develop evidence-based next-experiment suggestions for a scientist to review.','比较不同批次、发现信息缺口，并逐步形成可供研究人员审核的下一轮实验建议。'),
 stage:tx('LabPilot workflow software prototype; camera capture and recommendation workflows are development directions, not autonomous laboratory control.','LabPilot 工作流程软件处于原型阶段；相机采集和实验建议仍在开发规划中，不代表已实现实验室自主控制。'),
 image:'lab-concept.png',imageAlt:tx('Proposed external camera capturing observations through a sealed glovebox window','拟议的外置相机，通过密封手套箱观察窗采集实验影像'),
 caption:tx('Glovebox camera capture concept · generated visualization','手套箱视觉采集概念 · 生成式渲染'),
 secondary:'labpilot-screen.png',secondaryAlt:tx('LabPilot workflow-canvas software prototype screenshot','LabPilot 实验流程画布软件原型截图'),
 secondaryCaption:tx('Existing software prototype screenshot · demonstration data','既有软件原型截图 · 演示数据'),
 takeaway:tx('For research teams that need reliable records before better predictions.','面向希望先建立可靠记录，再实现更好预测的研发团队。'),
 link:tx('Discuss an R&D workflow','洽谈研发工作流程'),demo:'lab'
 },
